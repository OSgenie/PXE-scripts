        ...
        for release in $releases; do
          for i in {10..0}; do
            ...
            if [ $i == 0 ];then
              ...
              /usr/bin/wget --no-clobber -t 1 -T 1 $release_server"/"$release_folder"/"$distro"-"$release"-"$flavor"-"$arch".iso.torrent"
            else
              echo "checking "$release_server"/"$release_folder"/"$distro"-"$release"."$i"-"$flavor"-"$arch".iso.torrent"
              /usr/bin/wget --no-clobber -t 1 -T 1 $release_server"/"$release_folder"/"$distro"-"$release"."$i"-"$flavor"-"$arch".iso.torrent"
            fi
